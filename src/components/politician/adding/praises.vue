...
<div>
    <h1>Praises</h1>
                        ...
                        <v-card outlined>
                            <v-list-item >
                                ...
                                <v-list-item-content> 
                                    ...
                                    <!-- <v-file-input label="Insert image here" :rules="rules" placeholder="Select your preferred photo" accept="image/png, image/jpeg, image/bmp" v-model="praises.image" filled prepend-icon="mdi-camera"></v-file-input> 
        ...
        <v-row v-for="(praises,i) in tempArray" :key="praises._id" wrap>
            <v-col lg=12>
                <v-card outlined>
                    <v-list-item three-line>
                        ...
                        <v-list-item-content> 
                            <div class="overline mb-4">
                                Publisher: <v-text-field v-model="praises.publisher"></v-text-field>
                            </div>   
                            <v-list-item-title class="headline mb-1">
                                Title: <v-text-field v-model="praises.title"></v-text-field>
                            </v-list-item-title>
                            
                            <v-list-item-content>
                                Desciription: <v-text-field v-model="praises.description"></v-text-field>
                            </v-list-item-content>
                            <v-list-item-content>
                                Image source: <v-text-field v-model="praises.image"></v-text-field>
                        ...
                        <v-list-item-content> 
                            ...
                            <v-list-item-content>
                                {{praises.description}}... <a :href="praises.url">Read more</a>
                            </v-list-item-content>
                            
                        </v-list-item-content>
                        <v-list-item-avatar tile class="ma-3" size="125"><v-img :src="praises.image"></v-img></v-list-item-avatar>
                        </div>
                    </v-list-item>
                     <v-card-actions>
                    <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        async callAutoBackend(autoVar){
            try {
                console.log(autoVar)
                this.praisesData = await politicianService.getAutoVar(autoVar);
                if(autoVar=='' || null || !this.praisesData)
                {
                ...
                {
                   console.log(this.praisesData)
                    this.tempArray.push(this.praisesData);
                    this.autoVar='';
                    this.loadingText = false;
                }
            } catch (error) {
